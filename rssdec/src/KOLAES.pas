...
procedure ExpandAES128KeyForEncryption(const Key: TAESKey128; var ExpandedKey: TAESExpandedKey128);
procedure ExpandAES192KeyForEncryption(const Key: TAESKey192; var ExpandedKey: TAESExpandedKey192);
procedure ExpandAES256KeyForEncryption(const Key: TAESKey256; var ExpandedKey: TAESExpandedKey256);

// Block encryption routines

procedure EncryptAES128(const InBuf: TAESBuffer; const Key: TAESExpandedKey128; var OutBuf: TAESBuffer);
procedure EncryptAES192(const InBuf: TAESBuffer; const Key: TAESExpandedKey192; var OutBuf: TAESBuffer);
procedure EncryptAES256(const InBuf: TAESBuffer; const Key: TAESExpandedKey256; var OutBuf: TAESBuffer);

// Stream encryption routines (ECB mode)

procedure EncryptAES128StreamECB(Source: PStream; Count: longword; const Key: TAESKey128; Dest: PStream);
procedure EncryptAES128StreamECBEx(Source: PStream; Count: longword; const ExpandedKey: TAESExpandedKey128; Dest: PStream);

procedure EncryptAES192StreamECB(Source: PStream; Count: longword; const Key: TAESKey192; Dest: PStream);
procedure EncryptAES192StreamECBEx(Source: PStream; Count: longword; const ExpandedKey: TAESExpandedKey192; Dest: PStream);

procedure EncryptAES256StreamECB(Source: PStream; Count: longword; const Key: TAESKey256; Dest: PStream);
procedure EncryptAES256StreamECBEx(Source: PStream; Count: longword; const ExpandedKey: TAESExpandedKey256; Dest: PStream);

// Stream encryption routines (CBC mode)

procedure EncryptAES128StreamCBC(Source: PStream; Count: longword; const Key: TAESKey128; const InitVector: TAESBuffer; Dest: PStream);
procedure EncryptAES128StreamCBCEx(Source: PStream; Count: longword; const ExpandedKey: TAESExpandedKey128; const InitVector: TAESBuffer; Dest: PStream);

procedure EncryptAES192StreamCBC(Source: PStream; Count: longword; const Key: TAESKey192; const InitVector: TAESBuffer; Dest: PStream);
procedure EncryptAES192StreamCBCEx(Source: PStream; Count: longword; const ExpandedKey: TAESExpandedKey192; const InitVector: TAESBuffer; Dest: PStream);

procedure EncryptAES256StreamCBC(Source: PStream; Count: longword; const Key: TAESKey256; const InitVector: TAESBuffer; Dest: PStream);
procedure EncryptAES256StreamCBCEx(Source: PStream; Count: longword; const ExpandedKey: TAESExpandedKey256; const InitVector: TAESBuffer; Dest: PStream);

// Key transformation routines for decryption

procedure ExpandAES128KeyForDecryption(var ExpandedKey: TAESExpandedKey128);
procedure ExpandAES128KeyForDecryptionEx(const Key: TAESKey128; var ExpandedKey: TAESExpandedKey128);

procedure ExpandAES192KeyForDecryption(var ExpandedKey: TAESExpandedKey192);
procedure ExpandAES192KeyForDecryptionEx(const Key: TAESKey192; var ExpandedKey: TAESExpandedKey192);

procedure ExpandAES256KeyForDecryption(var ExpandedKey: TAESExpandedKey256);
procedure ExpandAES256KeyForDecryptionEx(const Key: TAESKey256; var ExpandedKey: TAESExpandedKey256);

// Block decryption routines

procedure DecryptAES128(const InBuf: TAESBuffer; const Key: TAESExpandedKey128; var OutBuf: TAESBuffer);
procedure DecryptAES192(const InBuf: TAESBuffer; const Key: TAESExpandedKey192; var OutBuf: TAESBuffer);
procedure DecryptAES256(const InBuf: TAESBuffer; const Key: TAESExpandedKey256; var OutBuf: TAESBuffer);

// Stream decryption routines (ECB mode)

procedure DecryptAES128StreamECB(Source: PStream; Count: longword; const Key: TAESKey128; Dest: PStream);
procedure DecryptAES128StreamECBEx(Source: PStream; Count: longword; const ExpandedKey: TAESExpandedKey128; Dest: PStream);

procedure DecryptAES192StreamECB(Source: PStream; Count: longword; const Key: TAESKey192; Dest: PStream);
procedure DecryptAES192StreamECBEx(Source: PStream; Count: longword; const ExpandedKey: TAESExpandedKey192; Dest: PStream);

procedure DecryptAES256StreamECB(Source: PStream; Count: longword; const Key: TAESKey256; Dest: PStream);
procedure DecryptAES256StreamECBEx(Source: PStream; Count: longword; const ExpandedKey: TAESExpandedKey256; Dest: PStream);

// Stream decryption routines (CBC mode)

procedure DecryptAES128StreamCBC(Source: PStream; Count: longword; const Key: TAESKey128; const InitVector: TAESBuffer; Dest: PStream);
procedure DecryptAES128StreamCBCEx(Source: PStream; Count: longword; const ExpandedKey: TAESExpandedKey128; const InitVector: TAESBuffer; Dest: PStream);

procedure DecryptAES192StreamCBC(Source: PStream; Count: longword; const Key: TAESKey192; const InitVector: TAESBuffer; Dest: PStream);
procedure DecryptAES192StreamCBCEx(Source: PStream; Count: longword; const ExpandedKey: TAESExpandedKey192; const InitVector: TAESBuffer; Dest: PStream);

procedure DecryptAES256StreamCBC(Source: PStream; Count: longword; const Key: TAESKey256; const InitVector: TAESBuffer; Dest: PStream);
procedure DecryptAES256StreamCBCEx(Source: PStream; Count: longword; const ExpandedKey: TAESExpandedKey256;  const InitVector: TAESBuffer; Dest: PStream);
...
begin
  ...
  repeat
    ...
    W2:=LastForwardTable[Byte(T shr 16)];
    W3:=LastForwardTable[Byte(T shr 24)];
    ExpandedKey[I+6]:=ExpandedKey[I] xor (W0 xor ((W1 shl 8) or (W1 shr 24)) xor
                      ((W2 shl 16) or (W2 shr 16)) xor ((W3 shl 24) or (W3 shr 8))) xor Rcon[J];
    Inc(J);
...
begin
  ...
  repeat
    ...
    W2:=LastForwardTable[Byte(T shr 16)];
    W3:=LastForwardTable[Byte(T shr 24)];
    ExpandedKey[I+8]:=ExpandedKey[I] xor (W0 xor ((W1 shl 8) or (W1 shr 24)) xor
                      ((W2 shl 16) or (W2 shr 16)) xor ((W3 shl 24) or (W3 shr 8))) xor Rcon[J];
    Inc(J);
    ...
    ExpandedKey[I+12]:=ExpandedKey[I+4] xor (W0 xor ((W1 shl 8) or (W1 shr 24)) xor
...
procedure EncryptAES128(const InBuf: TAESBuffer; const Key: TAESExpandedKey128; var OutBuf: TAESBuffer);
var T0,T1: array[0..3] of longword;
  ...
  for I:=1 to 9 do
    begin
      ...
      ExpandedKey[I*4]:=F2 xor F4 xor F8 xor (((F2 xor F9) shl 24) or
                        ((F2 xor F9) shr 8)) xor (((F4 xor F9) shl 16) or
                        ((F4 xor F9) shr 16)) xor ((F9 shl 8) or (F9 shr 24));
...
begin
  ...
  while Count>=sizeof(TAESBuffer) do
    begin
      ...
      if Done<sizeof(TempIn) then raise Exception.Create(E_InOut,SReadError);
      EncryptAES128(TempIn,ExpandedKey,TempOut);
      Done:=Dest.Write(TempOut,sizeof(TempOut));
      if Done<sizeof(TempOut) then raise Exception.Create(E_InOut,SWriteError);
      Dec(Count,sizeof(TAESBuffer));
    end;
  if Count>0 then
    begin
      Done:=Source.Read(TempIn,Count);
      if Done<Count then raise Exception.Create(E_InOut,SReadError);
...
begin
  ...
  DecryptAES128StreamECBEx(Source,Count,ExpandedKey,Dest);
end;
...
begin
  ...
  if Count=0 then Exit;
  if (Count mod sizeof(TAESBuffer))>0 then raise Exception.Create(E_Custom,SInvalidInBufSize);
  while Count>=sizeof(TAESBuffer) do
    begin
      Done:=Source.Read(TempIn,sizeof(TempIn));
      if Done<sizeof(TempIn) then raise Exception.Create(E_InOut,SReadError);
...
begin
  ExpandAES192KeyForDecryptionEx(Key,ExpandedKey);
  DecryptAES192StreamECBEx(Source,Count,ExpandedKey,Dest);
end;
...
begin
  ExpandAES256KeyForDecryptionEx(Key,ExpandedKey);
  DecryptAES256StreamECBEx(Source,Count,ExpandedKey,Dest);
end;
...
procedure EncryptAES128StreamCBCEx(Source: PStream; Count: longword; const ExpandedKey: TAESExpandedKey128; const InitVector: TAESBuffer; Dest: PStream);
var TempIn,TempOut,Vector: TAESBuffer;
    Done: longword;
begin
  if Count=0 then
    begin
...
procedure ExpandAES128KeyForDecryptionEx(const Key: TAESKey128; var ExpandedKey: TAESExpandedKey128);
begin
  ExpandAES128KeyForEncryption(Key,ExpandedKey);
  ExpandAES128KeyForDecryption(ExpandedKey);
end;

procedure ExpandAES192KeyForDecryptionEx(const Key: TAESKey192; var ExpandedKey: TAESExpandedKey192);
begin
  ExpandAES192KeyForEncryption(Key,ExpandedKey);
  ExpandAES192KeyForDecryption(ExpandedKey);
end;

procedure ExpandAES256KeyForDecryptionEx(const Key: TAESKey256; var ExpandedKey: TAESExpandedKey256);
begin
  ExpandAES256KeyForEncryption(Key,ExpandedKey);
  ExpandAES256KeyForDecryption(ExpandedKey);
end;
...
begin
  ExpandAES128KeyForEncryption(Key,ExpandedKey);
  EncryptAES128StreamECBEx(Source,Count,ExpandedKey,Dest);
end;
...
begin
  ExpandAES192KeyForEncryption(Key,ExpandedKey);
  EncryptAES192StreamECBEx(Source,Count,ExpandedKey,Dest);
end;
...
begin
  ExpandAES256KeyForEncryption(Key,ExpandedKey);
  EncryptAES256StreamECBEx(Source,Count,ExpandedKey,Dest);
end;
...
begin
  ExpandAES128KeyForEncryption(Key,ExpandedKey);
  EncryptAES128StreamCBCEx(Source,Count,ExpandedKey,InitVector,Dest);
end;
...
begin
  ExpandAES192KeyForEncryption(Key,ExpandedKey);
  EncryptAES192StreamCBCEx(Source,Count,ExpandedKey,InitVector,Dest);
end;
...
begin
  ExpandAES256KeyForEncryption(Key,ExpandedKey);
  EncryptAES256StreamCBCEx(Source,Count,ExpandedKey,InitVector,Dest);
end;
...
begin
  ExpandAES128KeyForDecryptionEx(Key,ExpandedKey);
  DecryptAES128StreamCBCEx(Source,Count,ExpandedKey,InitVector,Dest);
end;
...
begin
  ExpandAES192KeyForDecryptionEx(Key,ExpandedKey);
  DecryptAES192StreamCBCEx(Source,Count,ExpandedKey,InitVector,Dest);
end;
...
begin
  ExpandAES256KeyForDecryptionEx(Key,ExpandedKey);
  DecryptAES256StreamCBCEx(Source,Count,ExpandedKey,InitVector,Dest);
end;
